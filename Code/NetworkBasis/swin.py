...
"""
    copied from https://github.com/rishigami/Swin-Transformer-TF/
"""
...
class Mlp(tf.keras.layers.Layer):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., prefix=''):
        ...
        self.fc1 = KL.Dense(hidden_features, name=f'{prefix}/mlp/fc1')
        self.fc2 = KL.Dense(out_features, name=f'{prefix}/mlp/fc2')
...
def window_partition(x, window_size):
    B, H, W, L, C = x.get_shape().as_list()
    x = tf.reshape(x, shape=[-1, H // window_size,
                   window_size, W // window_size, window_size, L // window_size, window_size, C])
    x = tf.transpose(x, perm=[0, 1, 3, 5, 2, 4, 6, 7])
    windows = tf.reshape(x, shape=[-1, window_size, window_size,window_size, C])
    return windows


def window_reverse(windows, window_size, H, W, L, C):
    x = tf.reshape(windows, shape=[-1, H // window_size,
                   W // window_size, L // window_size, window_size, window_size, window_size, C])
    x = tf.transpose(x, perm=[0, 1, 4, 2, 5, 3, 6, 7])
    x = tf.reshape(x, shape=[-1, H, W, L, C])
...
class WindowAttention(tf.keras.layers.Layer):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., prefix=''):
        ...
        self.qkv = KL.Dense(dim * 3, use_bias=qkv_bias,
                         name=f'{self.prefix}/attn/qkv')
        self.attn_drop = KL.Dropout(attn_drop)
        self.proj = KL.Dense(dim, name=f'{self.prefix}/attn/proj')
    ...
    def build(self, input_shape):
        ...
        coords_t = np.arange(self.window_size[2])
        coords = np.stack(np.meshgrid(coords_h, coords_w,coords_t, indexing='ij'))
...
class SwinTransformerBlock(tf.keras.layers.Layer):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0, mlp_ratio=4.,
                 qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path_prob=0., norm_layer=KL.LayerNormalization, prefix=''):
        super().__init__()
        self.dim = dim
    ...
    def call(self, x):
        ...
        shortcut = x
        x = self.norm1(x)
        x = tf.reshape(x, shape=[-1, H, W, T, C])
        ...
        attn_windows = tf.reshape(
            attn_windows, shape=[-1, self.window_size, self.window_size, self.window_size, C])
        shifted_x = window_reverse(attn_windows, self.window_size, H, W, T, C)

        # reverse cyclic shift
        if self.shift_size > 0:
            x = tf.roll(shifted_x, shift=[
                        self.shift_size, self.shift_size, self.shift_size], axis=[1, 2, 3])
        else:
            x = shifted_x
        x = tf.reshape(x, shape=[-1, H * W * T, C])
...
class PatchMerging(tf.keras.layers.Layer):
    def __init__(self, input_resolution, dim, norm_layer=KL.LayerNormalization, prefix=''):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        self.reduction = KL.Dense(2 * dim, use_bias=False,
    ...
    def call(self, x):
        ...
        x = tf.reshape(x, shape=[-1, (H // 2) * (W // 2) * (T // 2), 4 * C])
...
class BasicLayer(tf.keras.layers.Layer):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path_prob=0., norm_layer=KL.LayerNormalization, downsample=None, use_checkpoint=False, prefix=''):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.depth = depth
...
class PatchEmbed(tf.keras.layers.Layer):
    def __init__(self, img_size=(224, 224, 224), patch_size=(4, 4, 4), in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__(name='patch_embed')
        patches_resolution = [img_size[0] //
                              patch_size[0], img_size[1] // patch_size[1], img_size[2] //
                              patch_size[2]]
        ...
        self.proj = KL.Conv3D(embed_dim, kernel_size=patch_size,
    ...
    def call(self, x):
        B, H, W, T, C = x.get_shape().as_list()
        assert H == self.img_size[0] and W == self.img_size[1] and T == self.img_size[2],\
            f"Input image size ({H}*{W}*{T}) doesn't match model ({self.img_size[0]}*{self.img_size[1]}*{self.img_size[2]})."
        x = self.proj(x)
        x = tf.reshape(
            x, shape=[-1, (H // self.patch_size[0]) * (W // self.patch_size[0])* (T // self.patch_size[0]), self.embed_dim])
...
class SwinTransformerModel(tf.keras.Model):
    def __init__(self, model_name='swin_tiny_patch4_window7_224', include_top=False,
                 img_size=(256, 256, 64), patch_size=(4, 4, 4), in_chans=2, num_classes=1000,
                 embed_dim=96, depths=[1,1,2], num_heads=[1,1,2],
                 window_size=5, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=KL.LayerNormalization, ape=False, patch_norm=True,
        ...
        self.pos_drop = KL.Dropout(drop_rate)
        ...
        self.norm = norm_layer(epsilon=1e-5, name='norm')
        self.avgpool = KL.GlobalAveragePooling1D()
        if self.include_top:
            self.head = KL.Dense(num_classes, name='head')
        else:
            self.head = None
...
def SwinAffine(model_name='swin_tiny_224', num_classes=1000, include_top=True, pretrained=True, use_tpu=False, cfgs=CFGS):
    cfg = cfgs[model_name]
    net = SwinTransformerModel(
        model_name=model_name, include_top=include_top, num_classes=num_classes, img_size=cfg['input_size'], window_size=cfg[
            'window_size'], embed_dim=cfg['embed_dim'], depths=cfg['depths'], num_heads=cfg['num_heads']
